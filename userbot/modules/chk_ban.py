...
async def _(e):
 if e.is_reply:
   ...
   await System.send_message(x, f"/{q} [{send}](th://user?id={send}) [{sad}](tg://user?id={sad}) // by {ll} ")""")
   await e.client.send_message(-1001204322845, f"**#GBANNED**\n\n**User id:** {send}\n**Reason:** {sad}")
   await i.edit(f'_User Traped_ [{send}](tg://user?id={send})')
 else:
...
async def _(e):
  ...
  if e.is_reply:
   ...
   await System.send_message(x, f"/{q} [{mm}](tg://user?id={mm}) // by [{hh}](tg://user?id={hh}) ")""")
    ...
    await e.reply(f"_Ban Lifted_ [{mm}](tg://user?id={mm})")
